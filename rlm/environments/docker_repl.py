...
# --- better-rlm: exec-protocol hardening -----------------------------------
...
RLM_RESULT_SENTINEL = "__RLM_RESULT__"

#: Cap on each echoed local's repr. Uncapped, a 7 MB context measured
#: 30,013,251 bytes of `locals` per call, and every derived slice added another
#: ~7 MB to every SUBSEQUENT call. Nothing reads the field in full.
LOCALS_REPR_CAP = 200
...
class LLMProxyHandler(BaseHTTPRequestHandler):
    ...
    def _handle_rlm_batched(self, body: dict) -> dict:
        ...
        # better-rlm: a session-fatal failure aborts the rest of the fan-out.
        fatal: list[str] = []

        def _run(index: int, prompt: str) -> None:
            if fatal:
                results[index] = f"Error: RLM query skipped - {fatal[0]}"
                return
            try:
                completion = self.subcall_fn(prompt, model)
                with comp_lock:
...
class DockerREPL(NonIsolatedEnv):
    ...
    def _write_owner_marker(self) -> None:
        """better-rlm: record who owns this sandbox so a later startup sweep can
        reap it if this process dies without running any exit hook.

        Third line is the host path of /workspace. The container cannot know it,
        yet anything the sandbox writes there is loadable from the host by that
        path — the route remotely-fetched data takes into a caller's store.
        Readers parse positionally, so a shorter marker still works.
        """
        try:
            with open(os.path.join(self.temp_dir, "owner"), "w",
    ...
    def execute_code(self, code: str) -> REPLResult:
        """Run one cell in the container.

        better-rlm hardening over the original: a uniquely-named, fsynced script
        (no rewrite-in-place race on the bind mount), a bounded wall clock, and an
        explicit exit-code / result-marker check. Previously a partial or killed
        run produced empty stdout, which was parsed as ``{}`` and reported as a
        successful 19-byte cell — a dead sandbox looked like a quiet one.
        """
        ...
        # already flushed and closed (the macOS virtiofs / Docker Desktop
        # bind-mount failure mode). encoding and newline are pinned because the
        # guest is Linux whatever the host is — without them a Windows host
        # writes cp1252 + CRLF into a UTF-8 reader.
        ...
        for line in reversed(out.splitlines()):
            if line.startswith(RLM_RESULT_SENTINEL):
                try:
                    data = json.loads(line[len(RLM_RESULT_SENTINEL):])
                except json.JSONDecodeError as exc:
                    err = f"{err}\nresult line unparseable ({exc})".strip()
                break

        if data is None:
            # The script is kept on disk for post-mortem: a silent empty result
            # was the whole bug, so surface where to look.
            reason = (
